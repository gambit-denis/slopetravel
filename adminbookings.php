...
<head>
    ...
    <title>SlopeTravel Admin - Rezervimet</title>
...
<body>
    ...
    <div class="menu">
        ...
        <div style="display: inline; justify-content:left; align-items: center; margin-bottom: 10px;">
                ...
                <h3>Menaxhimi i Rezervimeve</h3>
                </div>
            <button class="add-btn" onclick="window.location.href='adminbookings_new.php';"><i class="fa fa-plus"></i> Shto Rezervim</button>
            <table>
                <thead>
                    <tr>
                        <th>#</th>
                ...
                <tbody>
                    <?php
                        ...
                        if ($conn->connect_error) {
                            ...
                            $sql = "SELECT res.RESERVATIONID, ac.IDENTIFIER as NAME, CONCAT(c.IDENTIFIER,', ',cc.IDENTIFIER) as CITY, res.GUESTS, rty.IDENTIFIER as ROOMTYPE, res.STARTDATE, res.ENDDATE, res.ACTUALPRICE, res.LASTUPDATE, res.WEBSITE, stat.IDENTIFIER as STATUS FROM `RESERVATIONS` res join ROOMTYPES rty on rty.ROOMTYPEID = res.ROOMTYPEID join ACCOMMODATIONS ac on ac.ACCOMMODATIONID = res.ACCOMMODATIONID join CITIES c on c.CITYID = ac.CITYID join COUNTRIES cc on cc.COUNTRYID = c.COUNTRYID join STATES stat on stat.STATEID = res.STATEID where res.ISDELETED = 0;";
                            ...
                            if ($result === false) {
                                ...
                                echo "Nr. Rezervimeve: " . $result->num_rows . "<br>";
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        <a href='adminbookings_edit.php?reservationid=" . urlencode($row['RESERVATIONID']) . "' class='edit-btn'>
                                            <i class='fa fa-edit'></i>
                                        </a>
                                        <button class='delete-btn' onclick='deleteBooking(" . $row['RESERVATIONID'] . ")'>
                                            <i class='fa fa-trash-alt'></i>
                                        </button>
                                        <button class='web-btn' onclick='showBooking(\"" . htmlspecialchars($row["WEBSITE"], ENT_QUOTES, "UTF-8") . "\")'>
                                    ...
                                    echo "<tr><td colspan='6'>Nuk u gjet asnje rezervim!</td></tr>";
                                }
                            }
                            $conn->close();
                        }
    ...
    if (confirm("Jeni te sigurt qe deshironi te stornoni kete rezervim?")) {
        window.location.href = 'exec/deletereservation.php?reservationid=' + reservationid;
    }
}
    </script>
      <script>
        function showBooking(websiteURL) {
            if (websiteURL) {
